...
func GetInstallerDownloadLink() string {
	const BaseUrl = "https://github.com/chyzman/Chyzcord/releases/latest/download/"
	switch runtime.GOOS {
	case "windows":
		filename := Ternary(buildinfo.UiType == buildinfo.UiTypeCli, "ChyzcordInstaller.exe", "Chyzcord.exe")
		return BaseUrl + filename
	case "darwin":
		return BaseUrl + "Chyzcord.MacOS.zip"
	case "linux":
		return BaseUrl + "ChyzcordInstaller-linux"
	default:
		return ""
	}
}
...
func UpdateSelf() error {
	...
	tmp, err := os.CreateTemp(ownExeDir, "ChyzcordUpdate")
	if err != nil {
