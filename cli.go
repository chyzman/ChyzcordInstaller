...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install Chyzcord")
	var updateFlag = flag.Bool("repair", false, "Repair Chyzcord")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall Chyzcord")
	...
	if *versionFlag {
		fmt.Println("Chyzcord Installer", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
		fmt.Println("Copyright (C) 2023 Chyzman")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update Chyzcord' option to update, or run with --update-self")
			}
		}()
	...
	if install {
		...
		Log.Info("Downloading latest Chyzcord files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, Chyzcord will likely not work.")
}
